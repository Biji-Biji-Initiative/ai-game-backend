...
export class UIManager implements IUIManager {
  ...
  constructor(option: UIManagerOptions = {}) {
    ...
    this.container = null; // Property added
    this.toastContainer = null; // Property added
    this.loadingOverlay = null; // Property added
    this.modalContainer = null; // Property added
    
    this.eventListeners =  new Map(); // Property added
    this.elements =  new Map(); // Property added
    this.isInitialized = false; // Property added
    this.responseViewer = this.options.responseViewer || null; // Property added
  ...
  public initElements(): void {
    ...
    this.container =  document.getElementById(this.options.containerId || 'app-container'); // Property added
    ...
    this.toastContainer = toastContainer; // Property added
    ...
    this.loadingOverlay = loadingOverlay; // Property added
    ...
    this.modalContainer = modalContainer; // Property added
   ...
   */
  private closeToast(toas: HTMLElement): void {
   ...
   */
  public showSuccess(titl: string, message: string, duration: number = 4000): void {
    this.showToast({
      type: 'success',
      title,
      message,
   ...
   * Show an error toast
   */
  public showError(titl: string, message: string, duration: number = 5000): void {
    this.showToast({
      type: 'error',
      title,
      message,
   ...
   */
  public showWarning(titl: string, message: string, duration: number = 4000): void {
    this.showToast({
      type: 'warning',
      title,
      message,
   ...
   * Show an info toast
   */
  public showInfo(titl: string, message: string, duration: number = 3000): void {
    this.showToast({
      type: 'info',
      title,
      message,
   ...
   */
  public closeModal(modalE: HTMLElement): void {
   ...
   */
  public confirm(titl: string, message: string, onConfirm: () => void, onCancel?: () => void): void {
    this.showModal({
      title,
      content: `<p>${message}</p>`,
      size: 'small',
   ...
   * Set the UI theme
   */
  public setTheme(them: string): void {
    this.theme = theme; // Property added
  ...
  private setupEventListeners(): void {
    ...
    addEventListeners('.copy-response-btn', 'click', (event: Event) =>  {
      event.preventDefault();
    ...
    // Variable extraction
    addEventListeners('.extract-variable-btn', 'click', (event: Event) =>  {
      event.preventDefault();
    ...
    // Tab switching
    addEventListeners('.tab-button', 'click', (event: Event) =>  {
    ...
    addEventListeners('.collapsible-header', 'click', (event: Event) =>  {
   ...
   */
  private getFormData(for: HTMLFormElement): Record<string, unknown> {
   ...
   */
  private setLoadingIndicator(sho: boolean, message?: string): void {
   ...
   */
  private displayErrorMessage(titl: string, message: string, isHtml: boolean = false): void {
  ...
  /**
   * Shows a success message in the error container
   * @param message Success message
   * @param autoDismiss Whether to auto-dismiss after a timeout
   * @param dismissTimeout Timeout in ms before auto-dismissing
   */
  private displaySuccessMessage(messag: string, autoDismiss: boolean = true, dismissTimeout: number = 3000): void {
    const errorContainer = this.elements.get('error-container');
    if (!errorContainer) return;
   ...
   */
  showResponse(respons: Event, headers: Record<string, string> = {}, status: number = 200): void {
   ...
   */
  updateForm(dat: Record<string, unknown>): void {
   ...
   */
  addEventListener(even: string, callback: Function): void {
   ...
   * @param callback Callback function
   */
  removeEventListener(even: string, callback: Function): void {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
   ...
   */
  private emit(even: string, data[] | Record<string, unknown> = null): void {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
