...
// Get the appropriate port from environment variables
const getPort = () => {
    // First check API_PORT (our standardized variable)
    if (process.env.API_PORT) {
        return parseInt(process.env.API_PORT, 10);
    }

    // Then check PORT (common Node.js variable)
    if (process.env.PORT) {
        return parseInt(process.env.PORT, 10);
    }

    // Default based on environment
    return process.env.NODE_ENV === 'production' ? 9001 : 3001;
};
...
const config = {
    ...
    server: {
        ...
        baseUrl: process.env.BASE_URL || `http://localhost:${getPort()}`,
    },
    // API configuration
    api: {
