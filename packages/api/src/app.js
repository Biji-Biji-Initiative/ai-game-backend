...
// Add Sentry test routes for monitoring verification
import createSentryTestRoutes from "./core/infra/http/routes/sentry-test/sentryTestRoutes.js";
const sentryTestPath = `${config.api.prefix}/sentry-test`;
app.use(sentryTestPath, createSentryTestRoutes());
logger.info(`Sentry test routes available at ${sentryTestPath} (disabled in production)`);

// Add dedicated Sentry issues API endpoint
app.get('/api/v1/sentry-issues', async (req, res) => {
  try {
    // Forward to our sentry-test/issues endpoint
    const response = await fetch(`http://localhost:${config.port}${sentryTestPath}/issues`);
