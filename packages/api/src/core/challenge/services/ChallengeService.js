    ...
    constructor(dependencies = {}) {
        ...
        if (process.env.NODE_ENV === 'production') {
            if (!challengeRepository) {
                this.logger.warn('WARNING: ChallengeService running in production mode with mock repository. This is not recommended.');
            } else {
                this.logger.info('ChallengeService initialized in production mode with real repository');
            }
        } else {
            this.logger.info(`ChallengeService running in development mode with ${challengeRepository ? 'real' : 'mock'} repository`);
        }
