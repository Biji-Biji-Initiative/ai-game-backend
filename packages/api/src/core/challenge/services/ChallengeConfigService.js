...
""../../../challenge/errors/ChallengeErrors.js109;
""../../../infra/logging/domainLogger.js185;
'use strict';
const {
...
class ChallengeConfigService {
  ...
  constructor({
    ...
    this.logger = logger || challengeLogger.child({
      component: 'service:challenge-config'
    });
  ...
  async validateChallengeConfig(config) {
    ...
    if (config.challengeTypeCode) {
      validationPromises.push(this.getChallengeType(config.challengeTypeCode).catch(err => {
        errors.push(`Invalid challenge type: ${err.message}`);
      }));
    }
    // Validate format type if specified
    if (config.formatTypeCode) {
      validationPromises.push(this.getFormatType(config.formatTypeCode).catch(err => {
        errors.push(`Invalid format type: ${err.message}`);
      }));
    }
    // Validate focus area if specified
    if (config.focusArea) {
      validationPromises.push(this.getFocusAreaConfig(config.focusArea).catch(err => {
        errors.push(`Invalid focus area: ${err.message}`);
      }));
    }
    // Validate difficulty if specified
    if (config.difficulty) {
      validationPromises.push(this.getDifficultyLevel(config.difficulty).catch(err => {
...
export default ChallengeConfigService;"