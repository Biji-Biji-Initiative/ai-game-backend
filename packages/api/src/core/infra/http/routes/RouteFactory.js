...
class RouteFactory {
    ...
    createAuthRoutes() {
        try {
            // Try to get the auth controller from the container
            let authController = this.container.get('authController');

            // If no auth controller is available in development, create a mock
            if (!authController && process.env.NODE_ENV !== 'production') {
                // Create a basic mock auth controller for development
                authController = {
                    login: (req, res) => {
                        res.status(200).json({
                            success: true,
            ...
            // Make sure the authController has all required methods
            const requiredMethods = [
                'login', 'signup', 'refresh', 'logout',
                'forgotPassword', 'resetPassword', 'getStatus'
            ];

            // Validate controller and ensure all methods exist and are bound
            let controllerValid = true;
            if (authController) {
                requiredMethods.forEach(method => {
                    if (typeof authController[method] !== 'function') {
                        console.warn(`Auth controller missing method: ${method}`);
                        controllerValid = false;

                        // Add fallback method if in development
                        if (process.env.NODE_ENV !== 'production') {
                            authController[method] = (req, res) => {
                                res.status(501).json({
                                    success: false,
                    ...
                    // Ensure the method is bound to the controller instance
                    if (typeof authController[method] === 'function') {
            ...
            return import('./authRoutes.js').then(module => {
                ...
                return import('../middleware/validation.js').then(validation => {
                    // If controller is valid, use it to create routes
                    if (controllerValid) {
                        ...
                        // If controller is not valid, use fallback in development
                        if (process.env.NODE_ENV !== 'production') {
                            console.log('Using fallback auth routes due to invalid controller');

                            // Create a basic router with mock functionality
                            const router = express.Router();
                            ...
                            throw new Error('Auth controller missing required methods in production');
                        }
                    }
                });
            }).catch(error => {
    ...
    createUserRoutes() {
        ...
        if (!userController || typeof userController.getCurrentUser !== 'function') {
            console.error('Invalid userController, creating fallback router');
            const router = express.Router();

            // Different behavior based on environment
            if (process.env.NODE_ENV === 'production') {
                // In production, simple unavailable message
                router.all('*', (req, res) => {
                    res.status(503).json({
                ...
                // In development, more detailed info and some mock functionality
                router.get('/me', authenticateUser, (req, res) => {
                    // Auth check happens first, then service check
                    res.status(200).json({
                        success: true,
                        data: {
                            user: {
                                ...
                                fullName: 'Mock Development User',
                ...
                // Other development mock routes
                router.get('/', authenticateUser, (req, res) => {
                    res.status(200).json({
                        success: true,
                        data: {
                            users: [{ id: 'mock-id', email: 'dev@example.com', fullName: 'Dev User' }]
                        },
                        development: true
                    });
                });

                // For any other routes in dev, return service unavailable with details
                router.all('*', (req, res) => {
                    res.status(503).json({
                        error: 'User service unavailable',
                        message: 'The user service is currently unavailable',
                        development: true,
                        detail: 'Missing or invalid userController dependency'
                    });
                });
            }
            return router;
        ...
        try {
            ...
            return import('./userRoutes.js').then(module => {
                ...
                return Promise.all([
                    ...
                    // Create the router with resolved dependencies
                    // Authentication first, then service checks
                    const router = express.Router();

                    // Create a service availability middleware after authentication
                    const checkServiceAvailable = (req, res, next) => {
                        if (!userController || typeof userController.getCurrentUser !== 'function') {
                            return res.status(503).json({
                                error: 'User service unavailable',
                                message: process.env.NODE_ENV === 'production'
                                    ? 'This service is temporarily unavailable'
                                    : 'User controller is not properly configured',
                                requestId: req.id || req.headers['x-request-id']
                            });
                        }
                        return next();
                    ...
                    // Always use the actual createUserRoutes function
                    // We just intercept to add our service check middleware
                    const originalRouter = createUserRoutes({
                        userController,
                    ...
                    // Copy all route definitions but insert our middleware
                    originalRouter.stack.forEach(layer => {
                        if (layer.route) {
                            ...
                            route.stack.forEach(handler => {
                                ...
                                // Clone the stack but add our service check after auth
                                if (method) {
                                    const handlers = [...route.stack.map(h => h.handle)];

                                    // Insert service check after auth middleware
                                    // Find auth middleware index
                                    const authIndex = handlers.findIndex(h =>
                                        h.name === 'authenticateUser' ||
                                        h.toString().includes('authenticateUser')
                                    );

                                    if (authIndex !== -1) {
                                        // Insert after auth
                                        handlers.splice(authIndex + 1, 0, checkServiceAvailable);
                                    } else {
                                        // No auth found, add at beginning
                                        handlers.unshift(checkServiceAvailable);
                                    }

                                    // Register with proper method
                ...
                if (process.env.NODE_ENV === 'production') {
                    router.all('*', (req, res) => {
                        res.status(500).json({
                            error: 'Service error',
                            message: 'Unable to initialize user service',
                            requestId: req.id || req.headers['x-request-id']
                        });
                    });
                } else {
