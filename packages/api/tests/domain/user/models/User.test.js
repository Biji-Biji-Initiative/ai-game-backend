...
describe('User Domain Model', () => {
    ...
    beforeEach(() => {
        ...
        // Create a test user with Value Object
        testUser = new User({
            id: testUserId,
    ...
    describe('Account Status Management', () => {
        ...
        it('should change status to inactive when deactivated', () => {
            ...
            // Check that the event was published with Value Object ID
            expect(publishedEvents.length).to.equal(1);
            expect(publishedEvents[0].eventType).to.equal(EventTypes.USER_DEACTIVATED);
            expect(publishedEvents[0].payload.userId).to.equal(testUserId.value);
        });
    ...
    describe('Serialization', () => {
        it('should convert to database format correctly', () => {
            ...
            expect(dbData.id).to.equal(testUserId.value); // Check ID is serialized correctly
        ...
        it('should handle Value Objects in serialization', () => {
            const dbData = testUser.toDatabase();
            // ID should be serialized as primitive string
            expect(dbData.id).to.be.a('string');
            expect(dbData.id).to.equal(testUserId.value);
            
            // When converting back, should accept primitive and create Value Object
            const reconvertedUser = new User(dbData);
            expect(reconvertedUser.id).to.be.instanceOf(UserId);
            expect(reconvertedUser.id.value).to.equal(testUserId.value);
        });
    });
});
