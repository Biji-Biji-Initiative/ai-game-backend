...
export class NetworkError extends Error {
  ...
  constructor(message: string, status: number, statusText: string, response?: unknown) {
    super(message);
    this.name = 'NetworkError'; // Property added
...
function createResponseHandler<T>(responseType: string): ResponseHandler<T> {
  switch (responseType) {
    case 'json':
      return response => response.json() as Promise<T>;
    case 'text':
      return response => response.text() as Promise<T>;
    case 'blob':
      return response => response.blob() as Promise<T>;
    case 'arrayBuffer':
      return response => response.arrayBuffer() as Promise<T>;
    case 'formData':
      return response => response.formData() as Promise<T>;
    default:
      return response => response.json() as Promise<T>;
  }
}
