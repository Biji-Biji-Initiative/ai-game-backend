...
export class AuthManager {
  ...
  constructor(options = {}) {
    ...
    this.token = null; // Property added
    this.user = null; // Property added
  ...
  _loadFromStorage() {
    try {
      ...
      if (token) {
        this.token = token; // Property added
      }

      // Load user
      const userJson = localStorage.getItem(this.options.userStorageKey);
      if (userJson) {
        this.user = JSON.parse(userJson); // Property added
      }
  ...
  async login(email, password) {
    try {
      ...
      // Save token and user
      this.token = response.data.token; // Property added
      this.user = response.data.user; // Property added
