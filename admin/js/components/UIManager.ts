...
export class UIManager implements IUIManager {
   ...
   */
  public showSuccess(title: string, message: string, duration: number = 4000): void {
    this.showToast({
      type: 'success',
      title,
      message,
   ...
   */
  public closeModal(modalEl: HTMLElement): void {
  ...
  private setupEventListeners(): void {
    ...
    addEventListeners('.copy-response-btn', 'click', (event: MouseEvent) => {
      event.preventDefault();
    ...
    // Variable extraction
    addEventListeners('.extract-variable-btn', 'click', (event: MouseEvent) => {
      event.preventDefault();
    ...
    // Tab switching
    addEventListeners('.tab-button', 'click', (event: MouseEvent) => {
    ...
    addEventListeners('.collapsible-header', 'click', (event: MouseEvent) => {
   ...
   */
  private displayErrorMessage(title: string, message: string, isHtml: boolean = false): void {
   ...
   */
  private displaySuccessMessage(message: string, autoDismiss: boolean = true, dismissTimeout: number = 3000): void {
    const errorContainer = this.elements.get('error-container');
    if (!errorContainer) return;
   ...
   */
  showResponse(response: any, headers: Record<string, string> = {}, status: number = 200): void {
