...
export class UIManager {
  ...
  constructor(uiManager: NewUIManager) {
    ...
    if (!this.loadingOverlay) {
      ...
      this.loadingOverlay.className = 'fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 hidden';
  ...
  private cacheElements(): void {
    // Cache common elements
    const elementIds = [
      'app', 'flow-list', 'flow-steps', 'response-container',
      'variables-list', 'loading-overlay', 'toast-container'
    ];
   ...
   */
  showToast(title: string, message: string, type: string = 'info', duration: number = 5000): HTMLElement {
   ...
   * @returns The toast element
   */
  showError(title: string, message: string, duration: number = 8000): HTMLElement {
   ...
   * @returns The toast element
   */
  showSuccess(title: string, message: string, duration: number = 5000): HTMLElement {
