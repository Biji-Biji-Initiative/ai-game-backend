...
export class DomainStateViewer extends EventEmitter {
  ...
  jsonFormatter: JSONFormatterInterface | null = null;
  ...
  constructor(options: Partial<DomainStateViewerOptions> = {}) {
    ...
    // Assign properties directly from validated options
    this.options = options as DomainStateViewerOptions;
    ...
    if (typeof window !== 'undefined' && typeof window.JSONFormatter === 'function') {
      this.jsonFormatter = window.JSONFormatter as JSONFormatterInterface;
    }
  ...
  init() {
    ...
    const entityTypeFilter = this.container.querySelector(
      '#entity-type-filter',
    ) as HTMLInputElement;
    ...
    const stateSearchInput = this.container.querySelector(
      '#state-search-input',
    ) as HTMLInputElement;
    ...
    // Listen to domain state manager events using 'on'
    this.domainStateManager.on('snapshotChange', (data: unknown) => {
      if (typeof data === 'object' && data !== null && 'phase' in data && 'snapshots' in data) {
        const { phase, snapshots } = data as {
          phase: 'before' | 'after';
  ...
  toggleAllEntityTypes(selected: boolean) {
    const checkboxes = this.entityTypesListElement.querySelectorAll<HTMLInputElement>(
  ...
  updateSelectedEntityTypes() {
    ...
    this.takeAfterSnapshotButton.disabled =
      this.selectedEntityTypes.length === 0 ||
      !this.lastBeforeSnapshot ||
      Object.keys(this.lastBeforeSnapshot).length === 0;
  }
  ...
  private async handleTakeAfterSnapshot(): Promise<void> {
    ...
    try {
      ...
      this.showStatus(
        `Failed to take after snapshot: ${error instanceof Error ? error.message : String(error)}`,
        'error',
      );
    }
  }
  ...
  renderSnapshots(phase: string, snapshots: Record<string, StateSnapshot>) {
    ...
    Object.entries(snapshots).forEach(([type, data]) => {
      const entityTypeInfo = this.domainStateManager
        .getEntityTypes()
        .find((t: EntityType) => t.id === type) || { name: type };
  ...
  /**
   * Renders JSON data with collapsible sections
   * @param data The data to render
   * @returns HTML string
   */
  renderJsonWithCollapsibleSections(data: unknown): string {
    if (data === null || data === undefined) return '<div class="null-value">null</div>';

    if (this.jsonFormatter) {
      try {
        const formatter = new this.jsonFormatter(data, 5, {
        ...
        if (renderedElement instanceof HTMLElement) {
          ...
          logger.warn('JSONFormatter did not return an HTMLElement');
        }
      } catch (e) {
        logger.error('JSONFormatter failed:', e);
      }
    }
    // Fallback
    try {
      return `<pre class="snapshot-json">${this.escapeHtml(JSON.stringify(data, null, 2))}</pre>`;
    } catch (e) {
      return '<div class="error-message">Could not render JSON</div>';
    }
  }
   ...
   */
  renderChanges(change: Record<string, DiffChange>, path = ''): string {
    let html = '<ul class="changes-list">';

    Object.entries(change).forEach(([key, change]) => {
  ...
  formatValue(value: unknown): string {
    ...
    if (typeof value === 'string') {
      return `<span class="string-value">"${this.escapeHtml(value as string)}"</span>`;
    }
  ...
  showStatus(message: string, type = 'info') {
    if (!this.snapshotStatusElement) return;
