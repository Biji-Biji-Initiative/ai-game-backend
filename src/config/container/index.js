...
function validateDependencies(container, failFast = false) {
    ...
    const criticalDependencies = [
        ...
        // Constants
        // 'messageRole', // Removed: Using direct imports instead of container
    ...
    for (const dep of criticalDependencies) {
        try {
            ...
            missingDependencies.push({ name: dep, error: error.message });
        }
    }
    ...
    if (missingDependencies.length > 0) {
        ...
        if (failFast) {
            ...
            console.error('Missing:', missingDependencies.map(d => d.name).join(', '));
            process.exit(1);
        }
