...
async function findById(id) {
  try {
    const challenge = challengesDb.get(id);
    ...
    log('error', 'Error finding challenge by ID', { 
      id, 
      error: error.message 
    });
    throw error;
...
async function findByUserId(userId) {
  try {
    ...
    for (const challenge of challengesDb.values()) {
    ...
    log('error', 'Error finding challenges by user ID', { 
      userId, 
      error: error.message 
    });
    throw error;
 ...
 * @returns {Promise<Array<Challenge>>} Array of challenges
 */
async function findByFocusArea(focusArea) {
  try {
    const challenges = [];
    ...
    log('error', 'Error finding challenges by focus area', { 
      focusArea, 
      error: error.message 
    });
    throw error;
...
async function save(challenge) {
  try {
    ...
    if (!challenge.id) {
      challenge.id = (idCounter++).toString();
    }
    ...
    challengesDb.set(challenge.id, challenge);
    
    log('debug', 'Saved challenge', { 
      id: challenge.id, 
    ...
    log('error', 'Error saving challenge', { 
      error: error.message 
    });
    throw error;
  }
...
async function deleteById(id) {
  try {
    const existed = challengesDb.has(id);
    
    if (existed) {
      challengesDb.delete(id);
      
      log('debug', 'Deleted challenge', { id });
    }
 ...
 */
async function findByCriteria(criteria = {}, options = {}) {
  try {
    let matches = Array.from(challengesDb.values());
    ...
    log('error', 'Error finding challenges by criteria', { 
      criteria, 
      options, 
      error: error.message 
    });
