 ...
 * 
 * Handles HTTP requests related to difficulty levels.
 * Uses DTOs to format domain data for API responses.
 */

const { container } = require('../../../config/container');
const { DifficultyLevelDTOMapper } = require('../dtos/DifficultyLevelDTO');
const { applyControllerErrorHandling } = require('../../infra/errors/centralizedErrorUtils');
const { 
  ChallengeError, 
...
class DifficultyLevelController {
  ...
  constructor(difficultyLevelRepository = null) {
    ...
    this.getAllDifficultyLevels = applyControllerErrorHandling(
      this, 'getAllDifficultyLevels', 'challenge', errorMappings
    );
    
    this.getDifficultyLevelById = applyControllerErrorHandling(
      this, 'getDifficultyLevelById', 'challenge', errorMappings
    );
    
    this.createDifficultyLevel = applyControllerErrorHandling(
      this, 'createDifficultyLevel', 'challenge', errorMappings
    );
    
    this.updateDifficultyLevel = applyControllerErrorHandling(
      this, 'updateDifficultyLevel', 'challenge', errorMappings
    );
    
    this.deleteDifficultyLevel = applyControllerErrorHandling(
      this, 'deleteDifficultyLevel', 'challenge', errorMappings
    );
  }
