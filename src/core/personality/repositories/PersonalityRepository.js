...
const personalityMapper = require('../mappers/PersonalityMapper');
const { supabaseClient } = require('../../../core/infra/db/supabaseClient');
const { v4: uuidv4 } = require('uuid');
const { 
  personalityDatabaseSchema,
} = require('../schemas/personalitySchema');
...
class PersonalityRepository extends BaseRepository {
  ...
  constructor(options = {}) {
    ...
    this.deleteByUserId = this._wrapWithErrorHandling('deleteByUserId');
  }
  ...
  _wrapWithErrorHandling(methodName) {
    ...
    return async (...args) => {
      try {
        ...
        this.logger.error(`Error in personality repository ${methodName}`, {
          error: error.message,
          stack: error.stack,
          methodName,
          args: JSON.stringify(
        ...
        const mappedError = personalityErrorMapper(error, {
          methodName,
          domainName: 'personality',
          args,
        });
