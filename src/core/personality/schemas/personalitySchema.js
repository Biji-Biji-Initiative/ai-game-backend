...
const personalitySchema = z
  .object({
    ...
    aiAttitudes: z
      ...
      .refine(
        attitudes => Object.keys(attitudes).every(attitude => validAttitudes.includes(attitude)),
        { message: `AI attitudes must be one of: ${validAttitudes.join(', ')}` }
      )
      .default({}),
    dominantTraits: z
...
const attitudeUpdateSchema = z
  ...
  .refine(
    attitudes => Object.keys(attitudes).every(attitude => validAttitudes.includes(attitude)),
    { message: `Attitudes must be one of: ${validAttitudes.join(', ')}` }
  );
