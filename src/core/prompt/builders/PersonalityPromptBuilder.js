...
class PersonalityPromptBuilder {
  ...
  static build(params) {
    try {
      ...
      throw new PromptConstructionError(
        `Failed to build personality assessment prompt: ${error.message}`,
        {
          originalError: error,
        }
      );
  ...
  ||
    ...
    section.push(
      `The user has ${interactionHistory.length} recorded interactions with AI systems.`
    );
  ...
  static buildAnalysisGuidelinesSection(detailLevel, traitCategories) {
    ...
    if (detailLevel === 'comprehensive') {
      section.push(
        `Perform a comprehensive personality analysis focusing on the following categories:`
      );
