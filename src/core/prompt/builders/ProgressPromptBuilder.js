/**
 * Progress Prompt Builder
 * 
 * Specialized builder for generating progress assessment prompts that analyze
 * user learning journey, skill improvements, and provide personalized recommendations.
 * 
 * @module ProgressPromptBuilder
 * @requires logger
 * @requires apiStandards
 * @requires progressSchema
 */
...
class ProgressPromptBuilder {
  ...
  static build(params) {
    try {
      ...
      const { user, challengeAttempts = [], skillProgress = {}, options = {} } = validatedParams;
      ...
      if (challengeAttempts.length > 0) {
        ...
        const scores = challengeAttempts
...
module.exports = ProgressPromptBuilder; 