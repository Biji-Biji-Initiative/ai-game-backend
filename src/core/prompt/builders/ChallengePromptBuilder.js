...
const { appendApiStandards, getResponsesApiInstruction } = require('../common/apiStandards');
...
class ChallengePromptBuilder {
  ...
  static build(params) {
    try {
      ...
      const challengeType =
        challengeParams.challengeType || challengeParams.challengeTypeCode || 'standard';
      const formatType =
        challengeParams.formatType || challengeParams.formatTypeCode || 'open-ended';
  ...
  static buildUserProfileSection(user) {
    ...
    if (
      user.dominantTraits &&
      Array.isArray(user.dominantTraits) {
  &&
}
      user.dominantTraits.length > 0
    ) {
  ...
  static buildChallengeParametersSection(challengeParams, focusArea) {
    ...
    section.push(
      `Type: ${challengeParams.challengeType || challengeParams.challengeTypeCode || 'standard'}`
    );
    section.push(
      `Format: ${challengeParams.formatType || challengeParams.formatTypeCode || 'open-ended'}`
    );
  ...
  static buildGameStateSection(gameState) {
    ...
    if (
      gameState.recentChallenges &&
      Array.isArray(gameState.recentChallenges) {
  &&
}
      gameState.recentChallenges.length > 0
    ) {
      section.push(`\nRecent Challenges:`);
      gameState.recentChallenges.forEach((c, idx) => {
        section.push(
          `${idx + 1}. ${c.title} (${c.challengeType || 'Unknown type'}, ${c.difficulty || 'Not specified'})`
        );
      });
    }
    ...
    if (
      gameState.strengths &&
      Array.isArray(gameState.strengths) {
  &&
}
      gameState.strengths.length > 0
    ) {
      section.push(`\nUser Strengths: ${gameState.strengths.join(', ')}`);
    }

    if (
      gameState.areasForImprovement &&
      Array.isArray(gameState.areasForImprovement) {
  &&
}
      gameState.areasForImprovement.length > 0
    ) {
  ...
  static buildCreativitySection(creativeVariation, options) {
    ...
    if (options.allowDynamicTypes) {
      section.push(
        `- You may create novel challenge types beyond the standard categories when appropriate.`
      );
    }

    if (options.suggestNovelTypes) {
      section.push(
        `- You are encouraged to suggest creative and unique challenge types tailored to this specific user.`
      );
    }
  ...
  static buildAdaptationSection(gameState) {
    ...
    section.push(
      `- Create a challenge that builds on the user's strengths while addressing areas for improvement.`
    );
    ...
    if (gameState.streakCount && gameState.streakCount > 2) {
      section.push(
        `- The user is on a streak of ${gameState.streakCount} successfully completed challenges. Consider increasing difficulty slightly.`
      );
    }
    ...
    if (gameState.recentChallenges && gameState.recentChallenges.length > 0) {
      ...
      if (recentFocusAreas.length > 0 && !recentFocusAreas.includes(gameState.focusArea)) {
        section.push(
          `- This is a shift to a new focus area (${gameState.focusArea}) from previous work in ${recentFocusAreas.join(', ')}. Provide context that bridges these areas.`
        );
      }
    }
...

{
   ...
   * @private
   */
  static buildDynamicSystemMessage(
    challengeType,
    formatType,
    difficulty,
    user = {},
    ...
    try {
      ...
      // Base message
      messageParts.push(
        `You are an AI challenge creator specializing in ${challengeType} challenges `
      );
      ...
      if (formatType === 'open-ended') {
        messageParts.push(
          `You excel at creating open-ended challenges that promote creative thinking. `
        );
      } else if (formatType === 'scenario') {
        messageParts.push(
          `You excel at developing rich, realistic scenarios that require thoughtful analysis. `
        );
      } else if (formatType === 'multiple-choice') {
        messageParts.push(
          `You excel at crafting multiple-choice challenges with well-designed options. `
        );
      } else {
      ...
      if (user.skillLevel) {
        ...
        if (skillLevel === 'beginner') {
          messageParts.push(
            `Create challenges that build confidence while teaching fundamentals. `
          );
          ...
          messageParts.push(
            `Design sophisticated challenges that push the boundaries of expertise. `
          );
        }
      }
      ...
      if (personalityProfile && Object.keys(personalityProfile).length > 0) {
        if (personalityProfile.communicationStyle) {
          messageParts.push(
            `Adapt your communication style to be ${personalityProfile.communicationStyle}. `
          );
        }

        if (personalityProfile.learningPreferences) {
          messageParts.push(
            `Consider the user's learning preferences: ${personalityProfile.learningPreferences}. `
          );
        }
      }
