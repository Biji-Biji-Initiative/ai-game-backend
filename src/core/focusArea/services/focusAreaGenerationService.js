...
import { FocusAreaGenerationError } from "../errors/focusAreaErrors.js";
import errorStandardization from "../../core/infra/errors/errorStandardization.js";
import responseHandler from "../../core/infra/openai/responseHandler.js";
 ...
 * @requires logger
 */
const { focusAreaLogger } = domainLogger;
const { withServiceErrorHandling, createErrorMapper } = errorStandardization;
...
class FocusAreaGenerationService {
     ...
     * @param {Function} dependencies.formatJson - Function to format JSON responses
     * @param {Class} dependencies.FocusArea - FocusArea domain model class
     ...
     */
    constructor({ openAIClient, promptBuilder = require('../../prompt/promptBuilder'), formatJson = responseHandler.formatJson, FocusArea = focusArea, MessageRole, openAIStateManager, logger }) {
    ...
    async generateFocusAreas(userData, challengeHistory = [], progressData = {}, options = {}) {
        ...
        const conversationState = await this.openAIStateManager.findOrCreateConversationState(userId, `focus_area_${threadId}`, { createdAt: new Date().toISOString() });
