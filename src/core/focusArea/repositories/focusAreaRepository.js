...
const focusAreaLogger = logger.child({ service: 'focusAreaRepository' });
...
const createFocusArea = async (focusAreaData) => {
  try {
    ...
    if (error) {
      focusAreaLogger.error('Error creating focus area', {
        error: error.message,
        userId,
        name
      });
    ...
    focusAreaLogger.info('Created new focus area', {
      userId,
      focusAreaId: data.id,
      name,
    ...
    focusAreaLogger.error('Error in createFocusArea', {
      error: error.message,
      focusAreaData
    });
 ...
 */
const save = async (userId, focusAreas) => {
  try {
    focusAreaLogger.info('Saving focus areas for user', {
      userId,
    ...
    focusAreaLogger.debug('Formatted focus areas for insertion', {
      userId,
    ...
    if (error) {
      focusAreaLogger.error('Error creating multiple focus areas', {
        error: error.message,
        userId,
    ...
    focusAreaLogger.info('Created multiple focus areas', {
      userId,
      count: data.length,
    ...
    focusAreaLogger.error('Error in save', {
      error: error.message,
      userId,
...
const findByUserId = async (userId, options = {}) => {
  try {
    ...
    focusAreaLogger.error('Error in findByUserId', {
      error: error.message,
      userId
    });
...
const findById = async (id) => {
  try {
    ...
    if (error) {
      ...
      focusAreaLogger.error('Error getting focus area by ID', {
        error: error.message,
        id
      });
      throw new FocusAreaPersistenceError(error.message);
    ...
    focusAreaLogger.error('Error in findById', {
      error: error.message,
      id
    });
...
const update = async (id, updateData) => {
  try {
    ...
    const existingFocusArea = await findById(id);
    ...
    if (error) {
      focusAreaLogger.error('Error updating focus area', {
        error: error.message,
        id,
        updateData
      });
    ...
    focusAreaLogger.info('Updated focus area', {
      id,
      name: data.name,
      eventsPublished: domainEvents.length
    });
    ...
    focusAreaLogger.error('Error in update', {
      error: error.message,
      id,
      updateData
    });
...
const deleteAllForUser = async (userId) => {
  try {
    ...
    const focusAreas = await findByUserId(userId, { activeOnly: false });
    ...
    if (error) {
      focusAreaLogger.error('Error deleting focus areas for user', {
        error: error.message,
        userId
      });
      throw new FocusAreaPersistenceError(error.message);
    ...
    focusAreaLogger.info('Deleted all focus areas for user', { 
      userId,
      count: focusAreas.length,
      eventsPublished: allDomainEvents.length
    });
    ...
    focusAreaLogger.error('Error in deleteAllForUser', {
      error: error.message,
      userId
    });
...
const deleteById = async (id) => {
  try {
    ...
    const focusArea = await findById(id);
    if (!focusArea) {
    ...
    if (error) {
      focusAreaLogger.error('Error deleting focus area', {
        error: error.message,
        id
      });
      throw new FocusAreaPersistenceError(error.message);
    ...
    focusAreaLogger.info('Deleted focus area', { 
      id,
      eventsPublished: domainEvents.length
    });
    return true;
    ...
    focusAreaLogger.error('Error in deleteById', {
      error: error.message,
      id
    });
