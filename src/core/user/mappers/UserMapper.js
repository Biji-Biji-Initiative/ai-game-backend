 ...
 * User Mapper
 *
 * Handles conversion between User domain entities and persistence format.
...
class UserMapper {
  ...
  toDomain(data) {
    ...
    const lastActive =
      data.last_active instanceof Date
        ? data.last_active
        : data.last_active
  ...
  toPersistence(user) {
    ...
    return {
      ...
      last_active:
        user.lastActive instanceof Date ? user.lastActive.toISOString() : user.lastActive,
    };
  }
