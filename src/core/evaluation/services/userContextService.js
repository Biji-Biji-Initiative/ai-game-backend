...
class UserContextService {
  ...
  constructor(dependencies = {}) {
    ...
    // Apply service error handling
    applyServiceErrorHandling(this, evaluationServiceErrorMapper, [
      'gatherUserContext',
      'extractPersonalizedCriteria',
      'getDefaultCategoryWeights'
    ]);
  }
  ...
  async gatherUserContext(userId, options = {}) {
    try {
        ...
        try {
          ...
          if (recentEvaluations && recentEvaluations.length > 0) {
            ...
            if (recentEvaluations.length > 0) {
              context.learningJourney.averageScore = Math.round(
                totalScore / recentEvaluations.length
              );
            }
          }
      ...
      this.logger.error('Error gathering user context', {
        userId,
        error: error.message,
      });
  ...
  async extractPersonalizedCriteria(userContext, challenge) {
    try {
      ...
      const defaultWeights = this.getDefaultCategoryWeights(challenge);
      ...
      this.logger.error('Error extracting personalized criteria', { error: error.message });

      // Return default criteria on error
      return {
        categoryWeights: this.getDefaultCategoryWeights(challenge),
        focusAreas: [],
        skillLevel: 'intermediate',
        learningGoals: [],
        previousScores: {},
