 ...
 */
function generateDynamicPrompt({ challenge, userResponse, userContext, criteria, options = {} }) {
  try {
    // Build system message with personalized instructions
    const systemMessage = buildSystemMessage(challenge, userContext, criteria);
    
    // Build comprehensive user message
    const userMessage = buildUserMessage(challenge, userResponse, userContext, criteria);
    
    // Build response format instructions
    const responseFormat = buildResponseFormat(criteria);
    ...
    return {
      ...
      metadata: {
        ...
        userContextLevel: getUserContextLevel(userContext)
      }
    };
  } catch (error) {
    log('error', 'Error generating dynamic prompt', { 
      error: error.message,
 ...
 */
function buildSystemMessage(challenge, userContext, criteria) {
 ...
 */
function buildUserMessage(challenge, userResponse, userContext, criteria) {
  ...
  Object.entries(criteria.categoryWeights).forEach(([category, weight]) => {
    ...
    message += `- ${category} (0-${weight} points): ${description}\n`;
  });
 ...
 */
function buildResponseFormat(criteria) {
...
async function getCategoryDescription(category) {
  try {
    ...
    console.error('Error getting category description, using fallback', error);
 ...
 */
function getUserContextLevel(userContext) {
