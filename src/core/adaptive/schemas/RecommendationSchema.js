...
/**
 * Recommendation Schema Definitions using Zod
 *
 * Defines comprehensive validation schemas for Recommendation entities.
...
const challengeParametersSchema = z
  .object({
    ...
    customInstructions: z
      ...
      .refine(
        instr => !instr || instr.trim() === instr,
        'Custom instructions cannot start or end with whitespace'
      )
      .optional()
    ...
    recommendedChallengeTypes: z
      .array(
        z
          ...
          .refine(
            type => type.trim() === type,
            'Challenge type cannot start or end with whitespace'
          )
      )
      .default([])
