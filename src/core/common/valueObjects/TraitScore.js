...
class TraitScore {
  ...
  constructor(traitCode, score) {
    ...
    if (numericScore < TraitScore.MIN_SCORE || numericScore > TraitScore.MAX_SCORE) {
      throw new Error(`Score must be between ${TraitScore.MIN_SCORE} and ${TraitScore.MAX_SCORE}, got ${score}`);
    }
   ...
   */
  get intensityLevel() {
    if (this._score <= 2.5) return 'low';
    if (this._score <= 5.0) return 'moderate';
    if (this._score <= 7.5) return 'high';
    return 'very high';
  }
