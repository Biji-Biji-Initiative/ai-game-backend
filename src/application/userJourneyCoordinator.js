...
class UserJourneyCoordinator {
  ...
  async recordUserEvent(userEmail, eventType, eventData = {}, challengeId = null) {
    try {
      ...
      const userEvent = this.userJourneyService.formatUserEvent(
        userEmail, 
        eventType, 
        eventData, 
        challengeId
  ...
  async updateUserJourneyMetrics(userEmail, journeyMeta, recentEvents) {
    try {
      ...
      const challengeHistory = await this.challengeRepository.getUserChallengeHistory(userEmail);
      ...
      return await this.userRepository.updateUser(userEmail, { journeyMeta });
    } catch (error) {
