 ...
 *
 * This facade provides a unified interface to the specialized focus area coordinators,
 * maintaining backward compatibility with existing API contracts while allowing the
 * underlying implementation to follow Single Responsibility Principle (SRP).
 *
 * The facade pattern is used here to hide the complexity of multiple specialized
 * coordinators from client code, providing a simpler interface.
 *
 * @module FocusAreaCoordinatorFacade
 * @requires BaseCoordinator
 * @requires FocusAreaError
 * @requires FocusAreaGenerationCoordinator
 * @requires FocusAreaManagementCoordinator
 */

const BaseCoordinator = require('../BaseCoordinator');
const { FocusAreaError: _FocusAreaError } = require('../../core/focusArea/errors/focusAreaErrors');
