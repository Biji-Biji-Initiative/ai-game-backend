...
For questions, please format them as an array of objects with structure:
{
  "questions": [
    ...
    it('should generate challenges with different focus areas', async function () {
        // Define focus areas to test
        const focusAreas = ['Machine Learning', 'Data Privacy', 'AI Communication'];
...
It should include a scenario and questions that test basic understanding.`;
            ...
            if (response.data.focusArea) {
                expect(response.data.focusArea.toLowerCase()).to.include(focusArea.toLowerCase().split(' ')[0]);
            }
            // Or check keywords if focusArea doesn't match
            else if (response.data.keywords && Array.isArray(response.data.keywords)) {
                const matchesKeyword = response.data.keywords.some(keyword => keyword.toLowerCase().includes(focusArea.toLowerCase().split(' ')[0]));
