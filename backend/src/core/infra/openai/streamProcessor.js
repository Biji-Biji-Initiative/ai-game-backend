import { StreamEventType } from "#app/core/infra/openai/types.js";
import { OpenAIResponseHandlingError } from "#app/core/infra/openai/errors.js";
...
class StreamProcessor {
  ...
  constructor(options = {}) {
    ...
    this.logger = options.logger || apiLogger.child({
    ...
    try {
      ...
      switch (event.type) {
        ...
        default:
          ...
          this.logger.debug('Received unknown event type', {
   ...
   */
  /**
   * Method _handleFailed
   */
  _handleFailed(event) {
    this.status = 'failed';
    this.completed = true;
    if (event.response && event.response.error) {
      this.error = new OpenAIResponseHandlingError(event.response.error.message || 'API response failed', {
   ...
   */
  /**
   * Method _handleFunctionArgsDelta
   */
   ...
   */
  /**
   * Method _handleFunctionArgsDone
   */
  ...
  _handleError(event) {
    this.status = 'failed';
    const error = new OpenAIResponseHandlingError(event.message || 'Unknown streaming error', {
      code: event.code,
      context: event
    });
    this.error = error;
