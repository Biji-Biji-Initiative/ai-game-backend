 ...
 */
async function publishProgressUpdated(userEmail, area, value, metadata = {}, progressId) {
  try {
 ...
 * @returns {Promise<void>}
 */
async function publishAchievementUnlocked(userEmail, achievementId, achievementName, description, progressId) {
  try {
    // Get entity to add domain event
    const entity = await progressRepository.findById(progressId);
    if (entity) {
