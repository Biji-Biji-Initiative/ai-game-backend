...
// Use the Challenge type from the generated client
type Challenge = z.infer<typeof schemas.Challenge>;
...
export const generateChallenge = async (data: GenerateChallengeRequest): Promise<{
  ...
  const topTraits = data.personalityContext?.traits
    ?.sort((a, b) => b.value - a.value)
    .slice(0, 2) || [];
    
  const topTraitNames = topTraits.length > 0 
    ? topTraits.map(t => t.name).join(' and ')
    : '';
  
  // Get overall attitude context
  const attitudeContext = data.personalityContext?.attitudes
    ?.map(a => a.attitude)
    .join(' and ');
  ...
  if (professionalTitle) {
    roleContext = `as ${professionalTitle.includes('a ') || professionalTitle.includes('an ') ? professionalTitle : `a ${professionalTitle}`}`;
  } else {
    roleContext = 'with your background';
  }
  
  locationContext = location ? ` in ${location}` : '';
  ...
  const newChallenge: Challenge = {
    ...
    difficulty: data.difficulty as Challenge['difficulty'] || 'intermediate',
    status: 'pending',
    createdAt: now,
    updatedAt: now
  };
