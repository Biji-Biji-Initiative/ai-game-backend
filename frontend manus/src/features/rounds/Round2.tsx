...
// Define types from centralized type system
  ...
  const loadAIResponse = useCallback(async () => {
    ...
    try {
      ...
      if (aiResponseQuery.data?.success && aiResponseQuery.data?.data) {
        ...
        const responseData = aiResponseQuery.data.data as AIResponseData;
        
        // Extract content from the response - try different fields based on the API structure
        const content = responseData.content || responseData.response || '';
