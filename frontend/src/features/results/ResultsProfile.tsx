...
export default function ResultsProfile({ profileId }: { profileId?: string }) {
  ...
  const generateUserProfile = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        // First cast to unknown, then to Profile to satisfy TypeScript
        const profileData = result.data as unknown as Profile;
        ...
        setShareUrl(`${baseUrl}/profile/${profileData.id}`);
  ...
  useEffect(() => {
    if (isShared) {
      if (sharedProfileData?.success && sharedProfileData?.data) {
        // Ensure data is a valid Profile before setting
        if (typeof sharedProfileData.data === 'object' && sharedProfileData.data !== null && 'id' in sharedProfileData.data) {
          setProfile(sharedProfileData.data as unknown as Profile);
        }
      }
    } else if (gameProfile) {
  ...
  const { data: insightsResponse, isLoading: isLoadingInsights, error: insightsError } = useGeneratePersonalityInsights(!isShared); // Fetch insights only if not viewing a shared profile
             ...
             {insightsResponse?.success && insightsResponse.data && (
               <div className="space-y-8">
                 ...
                 <Card>
                   ...
                   <CardContent>
                     <p className="text-muted-foreground">{insightsResponse.data.overallSummary}</p>
                   </CardContent>
                 </Card>
                 ...
                 <div className="grid md:grid-cols-2 gap-6">
                   ...
                   <Card>
                     <CardHeader>
                       ...
                       <CardDescription>Primary: {insightsResponse.data.communicationStyle.primary} | Secondary: {insightsResponse.data.communicationStyle.secondary}</CardDescription>
                     </CardHeader>
                     <CardContent>
                       <p className="text-sm text-muted-foreground">{insightsResponse.data.communicationStyle.description}</p>
                     </CardContent>
                   </Card>
                   ...
                   <Card>
                     <CardHeader>
                       ...
                       <CardDescription>{insightsResponse.data.aiCollaborationStrategy.title}</CardDescription>
                     </CardHeader>
                     <CardContent>
                       <p className="text-sm text-muted-foreground mb-3">{insightsResponse.data.aiCollaborationStrategy.description}</p>
                       <h4 className="font-medium text-sm mb-1">Tips:</h4>
                       <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                         {insightsResponse.data.aiCollaborationStrategy.tips.map((tip, index) => (
                           <li key={index}>{tip}</li>
                         ))}
                       </ul>
                     </CardContent>
                   ...
                   <CardContent className="grid md:grid-cols-3 gap-4">
                     <div>
                       ...
                       <ul className="list-none space-y-1 text-sm text-muted-foreground">
                         {insightsResponse.data.workStyle.strengths.map((s, i) => <li key={i} className="flex items-start gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 mt-0.5 text-green-500 flex-shrink-0"/><span>{s}</span></li>)}
                       </ul>
                     </div>
                     <div>
                       <h4 className="font-medium mb-2 text-sm text-amber-700 dark:text-amber-300">Potential Challenges</h4>
                       <ul className="list-none space-y-1 text-sm text-muted-foreground">
                         {insightsResponse.data.workStyle.challenges.map((c, i) => <li key={i} className="flex items-start gap-1.5"><AlertCircle className="w-3.5 h-3.5 mt-0.5 text-amber-500 flex-shrink-0"/><span>{c}</span></li>)}
                       </ul>
                     </div>
                     <div>
                       <h4 className="font-medium mb-2 text-sm text-blue-700 dark:text-blue-300">Recommendations</h4>
                       <ul className="list-none space-y-1 text-sm text-muted-foreground">
                         {insightsResponse.data.workStyle.recommendations.map((r, i) => <li key={i} className="flex items-start gap-1.5"><Zap className="w-3.5 h-3.5 mt-0.5 text-blue-500 flex-shrink-0"/><span>{r}</span></li>)}
                       </ul>
                     </div>
                   </CardContent>
                 </Card>
                    ...
                    <Card>
                       ...
                       <CardContent>
                         <ul className="list-disc list-inside space-y-2 text-sm text-muted-foreground">
                            {insightsResponse.data.keyTraitInsights.map((insight, index) => (
                     ...
                     <Card>
                       ...
                       <CardContent>
                         <ul className="list-disc list-inside space-y-2 text-sm text-muted-foreground">
                            {insightsResponse.data.keyAttitudeInsights.map((insight, index) => (
                               <li key={index}>{insight}</li>
                            ))}
                         </ul>
                       </CardContent>
