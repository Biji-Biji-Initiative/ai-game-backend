 ...
 */
export const getUserProfile = (userId?: string): ApiResponse<UserProfile> => {
  // Validate input
  if (!userId) {
    return {
      success: false,
 ...
 */
export const updateUserProfile = (data?: { userId: string; data: UpdateProfileRequest }): ApiResponse<UserProfile> => {
  // Validate input
  if (!data || !data.userId) {
    return {
      success: false,
