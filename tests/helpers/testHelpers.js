...
function setupRealOpenAIClient() {
  if (!testEnv.hasRequiredVars('openai')) {
    console.warn('OPENAI_API_KEY not found, skipping real OpenAI client setup');
    return null;
  }
  
  const config = testEnv.getTestConfig().openai;
...
function skipIfMissingEnv(context, testType) {
  ...
  if (!testEnv.hasRequiredVars(testType)) {
...
module.exports = {
  ...
  createOpenAIStateManagerMock,
  setupRealOpenAIClient,
  createTestChallenge,
  createTestFocusArea,
  createTestUserProfile,
  verifyOpenAIResponseFormat,
  skipIfMissingEnv,
  wait,
  retry,
  MessageRole
