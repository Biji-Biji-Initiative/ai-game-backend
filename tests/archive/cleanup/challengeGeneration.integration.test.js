/**
 * Challenge Generation Integration Test
 * 
 * Tests the integration between challenge generation service and its dependencies
    ...
    challengeGenerationService = {
      generateChallenge: async (options = {}) => {
        ...
        const conversationState = await openAIStateManager.findOrCreateConversationState(
          userId,
          `challenge_generation_${threadId}`,
          { createdAt: new Date().toISOString() }
        );
    ...
    it('should generate challenges with different focus areas and difficulties', async function() {
      ...
      for (const { focusArea, difficulty } of combinations) {
        ...
        const matchingCall = promptBuilderCalls.find(call => 
          call.args[1].focusArea === focusArea && 
