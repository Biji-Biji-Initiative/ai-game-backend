/**
 * Evaluation Service - OpenAI Responses API Integration Test
 * 
 * Tests the integration between the evaluation service and OpenAI Responses API
...
// Set longer timeout for API tests
  ...
  beforeEach(function() {
    ...
    // Create the evaluation service with injected dependencies
    ...
    evaluationService = {
      evaluateResponse: async (challenge, userResponse, options = {}) => {
        ...
        const conversationState = await openAIStateManager.findOrCreateConversationState(
          challenge.userId,
  ...
  describe('Challenge Response Evaluation', function() {
    ...
    it('should handle challenges with different types and formats', async function() {
      ...
      for (const challenge of challenges) {
        ...
        const matchingCall = promptBuilderCalls.find(call => 
          call.args[1].challenge.id === challenge.id
        );
