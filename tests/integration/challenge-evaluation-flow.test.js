/**
 * Integration Test: Challenge-Evaluation Cross-Domain Flow
 * 
 * This test verifies the complete flow across Challenge and Evaluation domains:
 * 1. Generate a challenge using OpenAI
 * 2. Store it in Supabase
 * 3. Generate a response to the challenge
 * 4. Evaluate the response using OpenAI
 * 5. Store the evaluation in Supabase
 * 6. Retrieve both challenge and evaluation from database
 */
  ...
  it('should generate a challenge, simulate a response, evaluate it, and store everything in Supabase', async function() {
    try {
      ...
      try {
        ...
        const supabaseKey = process.env.SUPABASE_KEY || process.env.SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR2bWZwZGRtbnpheGpteHhwdXBrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDI2NDA3MTAsImV4cCI6MjA1ODIxNjcxMH0.99b38YXJbbNC8kjRpqQq96k0zaB5qwQ2vvcFdxHPH9Y';
      ...
      try {
        ...
        try {
          ...
          if (evalDeleteError || challengeDeleteError) {
            console.warn('Failed to delete some test data:', 
              evalDeleteError ? evalDeleteError.message : '', 
              challengeDeleteError ? challengeDeleteError.message : '');
          }
        } catch (cleanupError) {
