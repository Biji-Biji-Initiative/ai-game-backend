 ...
 * 
 * Tests the integration between the OpenAI client and state management
 * using proper mocking and dependency injection.
 */
...
// Use test setup helper to maintain consistency with other tests
...
describe('Integration: OpenAI Responses API Workflow', function() {
  ...
  beforeEach(async function() {
    ...
    if (process.env.NODE_ENV === 'test') {
      ...
      // Use the real client for integration tests with the actual API
    ...
    conversationState = await openAIStateManager.createConversationState(
      TEST_USER_ID,
      `test_${threadId}`,
      {
        testId: threadId,
...
3. Clarity of presentation`
  ...
  describe('Conversation State Management', function() {
    it('should create, find, update and delete conversation states', async function() {
      // Create a new state
      const newState = await openAIStateManager.createConversationState(
        TEST_USER_ID,
        'new_context',
        { test: true }
      );
      ...
      const foundState = await openAIStateManager.findOrCreateConversationState(
        TEST_USER_ID,
        'new_context',
        { test: true }
      );
