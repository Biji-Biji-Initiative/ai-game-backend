/**
 * Focus Area Generation Integration Test with Real APIs
 * 
 * This test validates the entire focus area generation flow using real API calls
...
// Set longer timeout for real API calls
  ...
  it('should generate focus areas using the actual service', async function() {
    ...
    const focusAreas = await focusAreaGenerationService.generateFocusAreas(
      testUser,
      challengeHistory,
      { skillLevels: { questioning: 7, instructions: 6 } },
      { 
        threadId, 
        count: 2, 
        temperature: 0.7
