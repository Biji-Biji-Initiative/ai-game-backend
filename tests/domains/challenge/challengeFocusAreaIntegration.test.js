/**
 * Cross-Domain Integration Tests - Challenge and Focus Area
 * 
 * Tests the integration between the Challenge and Focus Area domains
...
const FocusArea = require('../../../src/core/focusArea/models/FocusArea');
...
describe('Challenge and Focus Area Cross-Domain Integration', function() {
  ...
  beforeEach(function() {
    ...
    domainEvents.registerHandler('ChallengeCompleted', async (event) => {
      ...
      if (data.focusArea) {
        await focusAreaProgressService.updateProgress(
          data.userId,
          data.focusArea,
          data.score
        );
  ...
  it('should publish FocusAreaCompleted event when progress reaches 100%', async function() {
    ...
    await challengeService.completeChallenge(
      challenge.id,
      userId,
      'My excellent final response',
      95 // High score to ensure completion
    );
