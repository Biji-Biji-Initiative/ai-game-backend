...
/**
 * Tests the OpenAI Responses API integration with detailed logging
...
Format your response as valid, parsable JSON with no markdown formatting.`
    ...
    if (response.data.strengths) {
      response.data.strengths.forEach((strength, _i) => {
        console.log(`${_i+1}. ${strength}`);
      });
    }
    
    console.log('\nDetailed Strength Analysis:');
    if (response.data.strengthAnalysis) {
      response.data.strengthAnalysis.forEach((analysis, _i) => {
    ...
    if (response.data.areasForImprovement) {
      response.data.areasForImprovement.forEach((area, _i) => {
        console.log(`${_i+1}. ${area}`);
      });
    }
